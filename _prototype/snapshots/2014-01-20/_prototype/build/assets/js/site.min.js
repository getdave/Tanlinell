...
(function($) {
    (function() {
        function activateVisiblePicturefill(elm) {
            var redraw = false;
            var ele = $(elm);
            var needsToDisplay = !ele.attr("data-picture");
            if (needsToDisplay) {
                ele.attr("data-picture", "");
                redraw = true;
            }
            window.picturefill();
            setTimeout(function() {
                ele.addClass("is-loaded").css("padding-bottom", "");
            }, 1e3);
        }
        $(".lazyload").one("inview", function(event, isInView, visiblePartX, visiblePart) {
            if (isInView) {
                activateVisiblePicturefill($(this));
            }
        });
        $(document).ready(function() {
            var ratio = "56.25";
            $(".lazyload").each(function() {
                var ele = $(this);
                if (ele.attr("data-lazy-ratio")) {
                    ratio = ele.attr("data-lazy-ratio").split(":");
                    ratio = ratio[1] / ratio[0] * 100;
                }
                ele.css("padding-bottom", ratio + "%");
            });
        });
    })();
})(jQuery);