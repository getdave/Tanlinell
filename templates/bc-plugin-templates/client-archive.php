<?php
/**
  * Archive for CPT defined in the plugin.
  *
  *
  * This template to list all items of a cpt defined via bc_s_c_register_cpt
  *
  *
  * @function (obj)    BC_Simple_Clients::bc_s_c_list_obj();  public static function for retrieving cpt list
  */
?>
	...
	<?php do_action( 'bc_p_content_wrapper_start');?>
		<?php do_action( 'bc_p_content_main_start');?>		
			 ...
			 */
			?>	
			<?php $archive_post = get_page_by_path( BC_S_C_CPT_PLURAL_SLUG ); ?>
			...
			<?php $custom_posts_obj = BC_Simple_Clients::bc_s_c_list_obj(); ?>
			...
			<?php if ( $custom_posts_obj->have_posts() ): ?>	
				...
				<ul class="bc-sc-item-list bcp-item-list">
					...
					<li class="bc-sc-list-item bcp-list-item bcp-list-item--<?php echo ++$custom_posts_loop_count; ?><?php echo ($custom_posts_loop_count % $custom_posts_per_row == 1) ? ' bcp-list-item--first' : ''; ?>">
						<?php include( BC_S_C_T_P_DIR . BC_S_C_CPT_SLUG ."-listing-item.php"  ); ?>
					</li>		
					<?php endwhile; ?>	
				</ul>
